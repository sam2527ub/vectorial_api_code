...
class AudienceProfile:
    ...
    def __init__(self, row: Dict[str, Any]):
        ...
        self.linkedinUrl = row.get('linkedinUrl') or row.get('linkedinurl')
...
def create_audience_room(
    ...
    with get_audience_connection() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            cur.execute(
                """
                INSERT INTO "AudienceRoom" (id, name, "descriptionS3Url", "userId", "createdAt", "updatedAt")
                VALUES (%s, %s, %s, %s, %s, %s)
                RETURNING *
                """,
                (room_id, name, description_s3_url, user_id, now, now)
            )
            ...
            if profiles_data:
                for profile_data in profiles_data:
                    ...
                    cur.execute(
                        ...
                        (id, "audienceRoomId", "profileName", "linkedinUrl", "profileDescriptionS3Url", "postsS3Url", "createdAt", "updatedAt")
