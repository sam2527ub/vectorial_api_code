#!/usr/bin/env python3
"""
Final Predictions with User Backstory, Seed Backstory, and Memory
=================================================================

Generates final predictions for training data using user backstory, seed backstory, and memory from SGO training.
- Uses the same pipeline as pre_sgo_predictions
- Loads memory from _memory folder for each tribe
- Loads refined characteristics for each tribe
- Adds memory context, user backstory, and refined characteristics to prompts
- Includes: user backstory (user characteristics), seed backstory (original persona seed), user motives (memory), refined characteristics (persona profile), category characteristics

Usage:
    python 07_sgo_training/scripts/final_predictions_user_seed_memory.py
    python 07_sgo_training/scripts/final_predictions_user_seed_memory.py --only-cluster cluster_0
    python 07_sgo_training/scripts/final_predictions_user_seed_memory.py --only-cluster cluster_4 --only-micro micro_15
    python 07_sgo_training/scripts/final_predictions_user_seed_memory.py --start-from-cluster cluster_1
"""
...
def create_final_prompt_with_memory_and_refined_chars(
    ...
    """
    Create enhanced prompt with memory and refined characteristics included.
    Includes ALL context sources:
    - Refined characteristics (persona profile)
    - User characteristics (from user_backstories)
    - Category characteristics (from user_backstories)
    - User motives (from memory/batch_analyses - all analyses for this user)
...
"""
    ...
    # Format user characteristics section (from user_backstories)
    user_chars_section = ""
    if user_char_summary and user_char_summary.strip():
        user_chars_section = f"""
### 1. Your Personal Characteristics
**What Makes You Unique as a Reviewer:**
{user_char_summary}
"""
    
    # Format user motives section (from memory/batch_analyses)
    user_motives_section = ""
    if user_motives and user_motives.strip():
        user_motives_section = user_motives  # Already formatted by format_memory_for_prompt
    
    # Format category characteristics section
    category_section = ""
...
"""
    ...
    if user_chars_section:
        all_context_sections += user_chars_section
    if user_motives_section:
        all_context_sections += user_motives_section
    if category_section:
        all_context_sections += category_section
    ...
    if scoring_mode == "logprobs":
        prompt = prompt_template.format(
            ...
            memory_section=user_motives if user_motives else "No past learnings available.",
...
def process_single_review_with_memory_and_refined_chars(args_tuple, debug_print_context=False):
    ...
    # Validate user_motives is a string (optional - can be empty)
    if user_motives is None:
        user_motives = ""
    elif not isinstance(user_motives, str):
        logging.warning(f"  ⚠️  user_motives is not a string (type: {type(user_motives)}) for review {actual_review.get('asin', 'unknown')} - using empty string")
        user_motives = ""
    
    # Log if user_motives is being used (only for first review to avoid spam)
    if review_key.endswith('_review_0'):
        if user_motives.strip():
            logging.info(f"  ✅ Using user_motives (memory analyses) in prompt ({len(user_motives)} chars)")
        else:
            logging.info(f"  ℹ️  No user_motives (memory analyses) available for this user - continuing with other context")
    
    # Create prompt with ALL context sources:
    # - refined_chars_dict (persona profile)
    # - user_char_summary (user characteristics from user_backstories)
    # - category_char_summary (category characteristics)
    # - user_motives (from memory/batch_analyses - all analyses for this user)
...
class FinalPredictionsRefinedCharsOrchestrator(PipelineOrchestrator):
    ...
    def run_pipeline(self, dataset_type, tribe_seed_data, user_backstories, review_data, 
        ...
        for tribe_id, tribe_seed in tribe_seed_data.items():
            ...
            # Load refined characteristics (persona profile) - mode determines whether to use refined or seed
            ...
            for user_id in tribe_users:
                ...
                # 1. User characteristics (from user_backstories - overall_characteristics)
                user_story = user_backstories.get(user_id)
                user_chars = ""
                if user_story and user_story.overall_characteristics:
                    user_chars = user_story.overall_characteristics.influencing_characteristics_summary or ""
                
                # 2. User motives (from memory/batch_analyses - all analyses for this user)
                # Format memory for this specific user (filter analyses by user_id) - this is user_motives
                persona_name = tribe_seed.persona_name if hasattr(tribe_seed, 'persona_name') else f'persona_{tribe_id}'
                # Track if this is the first review for this cluster (for debug logging)
                is_first_review_in_cluster = (user_id == tribe_users[0] and len(all_review_tasks) == 0)
                user_motives = format_memory_for_prompt(memory_data, persona_name, user_id, debug=is_first_review_in_cluster) if memory_data else ""
                
                if user_motives:
                    logging.info(f"  ✅ Loaded user motives (memory analyses) for {user_id}: {len(user_motives)} chars")
                else:
                    logging.warning(f"  ⚠️  User motives (memory analyses) not found for {user_id} - continuing without it")
                
                # 3. Category characteristics (loaded per review below)
                # 4. Seed backstory (loaded once per tribe above)
                
                # Log context sources
                logging.info(f"  📝 User {user_id} context sources:")
                logging.info(f"     - User chars: {'✅' if user_chars else '❌'}")
                logging.info(f"     - User motives (memory): {'✅' if user_motives else '❌'} ({len(user_motives)} chars)" if user_motives else f"     - User motives (memory): ❌ (not found - continuing without it)")
                if user_chars:
                    logging.info(f"       User chars preview: {user_chars[:150]}...")
                if user_motives:
                    logging.info(f"       User motives preview: {user_motives[:150]}...")
                ...
                for review_idx, review in enumerate(reviews):
                    ...
                    if topic_probs is None:
                        error_msg = f"CRITICAL ERROR: topic_probabilities is missing for review (asin: {review.get('asin', 'unknown')}, user: {user_id}, category: {cat})"
                        logging.error(error_msg)
                        raise ValueError(error_msg)
                    ...
                    if is_first_review_in_cluster and review_idx == 0:
                        ...
                        logging.info(f"\n--- USER CHARACTERISTICS (from user_backstories) ---")
                        logging.info(f"User Chars: {user_chars[:300] if user_chars else 'N/A'}...")
                        logging.info(f"\n--- USER MOTIVES (from memory/batch_analyses) ---")
                        if user_motives:
                            logging.info(f"User Motives length: {len(user_motives)} chars")
                            logging.info(f"User Motives (FULL TEXT):")
                            logging.info(f"{'='*80}")
                            logging.info(user_motives)
                            logging.info(f"{'='*80}")
                        else:
                            logging.info("User Motives: No memory analyses available")
                        logging.info(f"\n--- CATEGORY CHARACTERISTICS ---")
                        logging.info(f"Category Summary: {cat_summary[:300] if cat_summary else 'N/A'}...")
                        logging.info(f"\n--- PRODUCT INFO ---")
                    ...
                    # Add user_motives (memory analyses filtered by user_id), refined_chars_dict (replaces persona_context), review_key, embeddings_cache, and lock to task tuple
                    # Pass ALL context sources: user_chars, user_motives (memory), seed_backstory
...
def main():
    parser = argparse.ArgumentParser(description='Generate final predictions with user backstory, seed backstory, and memory from SGO training')
    ...
    output_artifact = "sgo_train_final_predictions_user_seed_memory"
    logging.info(f"Output artifact name: {output_artifact}")
    
    # Initialize W&B run
    run = init_wandb_run(
        run_name="final_predictions_user_seed_memory",
        stage="07_sgo_training",
        config={
            "description": "Generate final predictions with user backstory, seed backstory, and memory",
    ...
    try:
        ...
        # Auto-detect test mode: if target cluster exists in test data directory, use test mode
        project_root = Path(__file__).parent.parent.parent
        local_test_data_path = project_root / "ground_truth_test_micro_cluster_details_converted"
        if args.only_cluster and local_test_data_path.exists():
            # Check if this cluster exists in test data
            cluster_test_dir = local_test_data_path / args.only_cluster
            if cluster_test_dir.exists():
                if args.only_micro:
                    # Check specific micro cluster
                    micro_file = cluster_test_dir / f"{args.only_micro}_details.json"
                    if micro_file.exists():
                        dataset_type = "test"
                        logging.info(f"✅ Auto-detected test mode: {args.only_cluster}/{args.only_micro} exists in test data directory")
                else:
                    # Check if any micro clusters exist in this cluster
                    micro_files = list(cluster_test_dir.glob("micro_*_details.json"))
                    if micro_files:
                        dataset_type = "test"
                        logging.info(f"✅ Auto-detected test mode: {args.only_cluster} has {len(micro_files)} micro clusters in test data directory")
        ...
        if dataset_type == "test":
            # For test mode: try local ground_truth_test_micro_cluster_details_converted first, then W&B
            project_root = Path(__file__).parent.parent.parent
            local_test_data_path = project_root / "ground_truth_test_micro_cluster_details_converted"
            
            if local_test_data_path.exists():
                test_data_path = local_test_data_path
                logging.info(f"✅ Using local test data from: {test_data_path}")
            else:
                # Fall back to W&B artifact
                sgo_input_artifacts = sgo_config.get("input_artifacts", {}) if sgo_config else {}
                test_data_artifact = sgo_input_artifacts.get("ground_truth_test_micro_cluster_details") or sgo_input_artifacts.get("test_micro_cluster_details")
                if not test_data_artifact:
                    logging.error("❌ ground_truth_test_micro_cluster_details or test_micro_cluster_details not found in 07_sgo_training config (required for test mode)")
                    logging.error(f"Available input_artifacts in 07_sgo_training config: {list(sgo_input_artifacts.keys())}")
                    return
            ...
            # Determine target cluster/micro from arguments (before loading test data)
            target_cluster_id = None
            target_micro_id = None
            if args.only_cluster:
                target_cluster_id = args.only_cluster
                if args.only_micro:
                    target_micro_id = args.only_micro
                    logging.info(f"Will load test data only for {target_cluster_id}/{target_micro_id}")
                else:
                    logging.info(f"Will load test data only for {target_cluster_id} (all micro clusters)")
            ...
            for cluster_dir in sorted(test_data_path.iterdir()):
                ...
                # Filter by target cluster if specified
                if target_cluster_id and cluster_id != target_cluster_id:
                    continue
                
                for micro_file in sorted(cluster_dir.glob('micro_*_details.json')):
                    micro_id = micro_file.stem.replace('_details', '')
                    
                    # Filter by target micro if specified
                    if target_micro_id and micro_id != target_micro_id:
                        continue
                    ...
                    for user_id, user_reviews in members_by_user.items():
                        ...
                        # Process each review - each review MUST have topic_probabilities
                        for review in user_reviews:
                            ...
                            if topic_probs is None:
                                ...
                                error_msg = f"CRITICAL ERROR: Review {asin} in {cluster_id}/{micro_id} for user {user_id} missing topic_probabilities"
                                logging.error(error_msg)
                                raise ValueError(error_msg)
        ...
        characteristics_mode = sgo_config.get("characteristics_mode", "seed") if sgo_config else "seed"
