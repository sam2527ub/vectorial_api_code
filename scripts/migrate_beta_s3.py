"""
Migration script for beta database - ONLY migrates files for rooms in beta database.
This script:
1. Gets all rooms from beta database (ONLY)
2. For each room, finds ALL files in old S3 locations
3. Copies all files to new location (skips if already exists)
4. Updates database URLs to point to new location (skips if already correct)
...
def main():
    logger.info("="*60)
    logger.info("Beta S3 Migration - Database-Driven")
    logger.info(f"Enterprise: {ENTERPRISE_NAME}")
    logger.info("="*60)
    logger.info("ONLY migrating files for rooms in beta database")
    logger.info("ONLY updating AudienceRoom and AudienceProfile tables")
    logger.info("Skipping rooms already in correct location")
    logger.info("NOT deleting anything")
    logger.info("="*60)
    
    # Get all rooms from beta database
    try:
        ...
        logger.info(f"Found {len(rooms)} rooms in beta database")
        
        if not rooms:
            logger.warning("No rooms found in beta database!")
            return
        ...
        for idx, room in enumerate(rooms, 1):
            try:
                ...
                # Check if room is already in correct location
                ...
                if already_correct:
                    logger.info(f"  ⏭ Skipped - already in correct location")
                    rooms_skipped += 1
                    continue
        ...
        logger.info(f"Rooms skipped (already correct): {rooms_skipped}")
        logger.info("="*60)
        logger.info("✓ Migration complete!")
