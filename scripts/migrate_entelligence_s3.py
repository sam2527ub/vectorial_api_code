"""
Migration script for entelligence database - ONLY migrates files for rooms in entelligence database.
This script:
1. Gets all rooms from entelligence database (ONLY)
2. For each room, finds ALL files in old S3 locations
3. Copies all files to new location
...
def main():
    logger.info("="*60)
    logger.info("Entelligence S3 Migration - Database-Driven")
    logger.info(f"Enterprise: {ENTERPRISE_NAME}")
    logger.info("="*60)
    logger.info("ONLY migrating files for rooms in entelligence database")
    logger.info("ONLY updating AudienceRoom and AudienceProfile tables")
    logger.info("NOT deleting anything")
    logger.info("="*60)
    
    # Get all rooms from entelligence database
    try:
        ...
        logger.info(f"Found {len(rooms)} rooms in entelligence database")
        
        if not rooms:
            logger.warning("No rooms found in entelligence database!")
            return
