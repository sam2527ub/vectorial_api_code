...
from app.services.dynamic_context_window_management_service.available_context_calculator import ContextCalculator
from app.services.dynamic_context_window_management_service.content_adjustment_handler import ContentAdjustmentHandler
from app.services.dynamic_context_window_management_service.batch_optimization_handler import BatchOptimizationHandler
...
class DynamicContextManager:
    ...
    def __init__(
        ...
        self.content_handler = ContentAdjustmentHandler(
            self.token_counter, self.content_adjuster, self.model_resolver
        )
        self.batch_handler = BatchOptimizationHandler(
            self.token_counter, self.batch_calculator, self.model_resolver, self.config
        )
    ...
    def adjust_content_to_fit_context_window(
        ...
        return self.content_handler.adjust_content(
            content, system_message, model_name, available_context, max_completion_tokens
        )
    ...
    def calculate_optimal_batch_sizes_for_activities(
        ...
        return self.batch_handler.calculate_optimal_batches(
            activity_text_list, system_message, model_name,
            available_context, max_completion_tokens, model_context_window
        )
