...
Job metadata stored in DB (CommentScrapeJob table). LinkedIn URLs from room profiles; on success update AudienceProfile.commentsS3Url.
"""
...
class UserCommentsFetchHandler:
    ...
    def start_run(
        ...
        try:
            db_job = database.create_comment_scrape_job(
            ...
            logger.exception("Failed to save comment job to database")
            raise HTTPException(status_code=500, detail=f"Failed to save job to database: {e}")
    ...
    def check_status(
        ...
        """
        Load job from DB by audience_room_id. Poll Apify for stored run_ids.
        When all SUCCEEDED, merge datasets and update AudienceProfile.commentsS3Url; update job in DB.
        """
        self._validate()
        room = database.find_audience_room_by_id(
        ...
        if not room:
            raise HTTPException(status_code=404, detail=f"Audience room {audience_room_id} not found")
        job = database.find_latest_comment_scrape_job_by_audience_room_id(
            audience_room_id=audience_room_id,
            enterprise_name=enterprise_name,
        )
        if not job:
        ...
        if not run_ids:
            return {
                ...
                "status": "FAILED",
                "profiles_count": 0,
                "error": job.error or "No run_ids in job",
            }
        ...
        new_result = {**result, "batches_completed": completed}
        ...
        if failed > 0 and failed + completed == len(run_ids):
            try:
                database.update_comment_scrape_job(
                    ...
                    {
                        "status": "FAILED",
                        "error": first_error or "One or more batch runs failed",
                        "result": new_result,
                    },
                    enterprise_name=enterprise_name,
                )
        ...
        if not raw_items:
            final_result = {"comments_found": 0, "profiles_updated": 0}
            try:
                database.update_comment_scrape_job(
                    job_id,
                    {
                        "status": "SUCCEEDED",
                        "error": None,
                        "result": {**new_result, "final_result": final_result},
                    },
                    enterprise_name=enterprise_name,
                )
            except Exception:
        ...
        process_result = process_comments_and_update_profiles(
            raw_items=raw_items,
            audience_room_id=audience_room_id,
            enterprise_name=enterprise_name,
        )
        try:
            database.update_comment_scrape_job(
                job_id,
                {
                    "status": "SUCCEEDED",
                    "error": None,
                    "result": {**new_result, "final_result": process_result},
                },
                enterprise_name=enterprise_name,
            )
        except Exception:
        ...
        return {
            ...
            "comments_found": process_result.get("comments_found", 0),
            "profiles_updated": process_result.get("profiles_updated", 0),
