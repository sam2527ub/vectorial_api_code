#!/usr/bin/env python3
"""
Final Predictions with User Backstory and Seed Backstory
========================================================

Generates final predictions for training data using user backstory and seed backstory as context.
- Uses the same pipeline as pre_sgo_predictions
- Uses seed characteristics for each tribe (persona profile) - NO refined characteristics
- EXCLUDES user motives (memory/batch_analyses) from context
- EXCLUDES category characteristics from context
- Includes: user backstory (user characteristics) + seed backstory (original persona seed)

Usage:
    python 07_sgo_training/scripts/final_predictions_user_seed_backstory.py
    python 07_sgo_training/scripts/final_predictions_user_seed_backstory.py --only-cluster cluster_0
    python 07_sgo_training/scripts/final_predictions_user_seed_backstory.py --only-cluster cluster_4 --only-micro micro_15
    python 07_sgo_training/scripts/final_predictions_user_seed_backstory.py --start-from-cluster cluster_1
"""
...
def create_final_prompt_with_memory_and_refined_chars(
    ...
    """
    Create enhanced prompt with user backstory, category backstory, and seed backstory.
    Includes context sources (EXCLUDES user motives):
    ...
    NOTE: User motives (memory) are EXCLUDED from context
    """
...
"""
    ...
    # Combine all context sections (user backstory + category backstory + seed backstory - excludes user motives)
...
def process_single_review_with_memory_and_refined_chars(args_tuple, debug_print_context=False):
    ...
    if review_key.endswith('_review_0'):
        logging.info(f"  ℹ️  Using user backstory + category backstory + seed backstory as context (user motives EXCLUDED)")
    
    # Create prompt with user backstory, category backstory, and seed backstory (EXCLUDES user motives):
    # - refined_chars_dict (persona profile)
    # - user_char_summary (user characteristics from user_backstories)
    # - category_char_summary (category characteristics from user_backstories)
    # - seed_backstory (original persona seed)
    # NOTE: user_motives (memory) is EXCLUDED - passed as empty string
...
class FinalPredictionsRefinedCharsOrchestrator(PipelineOrchestrator):
    ...
    def __init__(self, run, client, rate_limiter, prompt_template, config_params, 
                 category_mapping=None, memory_base_dir=None, user_learnings_base_dir=None):
    ...
    def run_pipeline(self, dataset_type, tribe_seed_data, user_backstories, review_data, 
        ...
        """Main execution method with memory loading (using seed characteristics, no refined)."""
        logging.info(f"Starting final predictions pipeline for {dataset_type.upper()} set with memory (seed characteristics only)")
        ...
        for tribe_id, tribe_seed in tribe_seed_data.items():
            ...
            # Use seed characteristics (persona profile) - always use seed, no refined characteristics
            ...
            if refined_chars_dict and refined_chars_dict.get('persona_name'):
                logging.info(f"  ✅ Using SEED characteristics for {cluster_id}/{micro_id}")
            else:
            ...
            for user_id in tribe_users:
                ...
                # Load context sources (user backstory + category backstory + seed backstory - EXCLUDES user motives):
                ...
                # NOTE: User motives (memory) are EXCLUDED - not loaded
                user_motives = ""  # EXCLUDED: user motives (memory) not used as context
                
                # 2. Category characteristics (loaded per review below)
                # 3. Seed backstory (loaded once per tribe above)
                
                # Log context sources (user backstory + category backstory + seed backstory)
                logging.info(f"  📝 User {user_id} context sources (user backstory + category backstory + seed backstory - excludes user motives):")
                logging.info(f"     - User backstory: {'✅' if user_chars else '❌'}")
                logging.info(f"     - Category characteristics: ✅ (loaded per review)")
                ...
                for review_idx, review in enumerate(reviews):
                    ...
                    # Load category characteristics (from user_backstories)
                    cat_summary = ""
                    ...
                    if is_first_review_in_cluster and review_idx == 0:
                        ...
                        logging.info(f"\n--- PERSONA CHARACTERISTICS (SEED - no refined characteristics) ---")
                    ...
                    # Pass context sources (user backstory + category backstory + seed backstory - excludes user motives): user_chars, cat_summary, user_motives (empty), seed_backstory
...
def main():
    parser = argparse.ArgumentParser(description='Generate final predictions with user backstory and seed backstory (seed characteristics only, no refined)')
    ...
    output_artifact = "sgo_train_final_predictions_user_seed_backstory"
    logging.info(f"Output artifact name: {output_artifact}")
    
    # Initialize W&B run
    run = init_wandb_run(
        run_name="final_predictions_user_seed_backstory",
        stage="07_sgo_training",
        config={
            "description": "Generate final predictions with user backstory and seed backstory (seed characteristics only, EXCLUDES user motives, category characteristics, and refined characteristics)",
    ...
    try:
        ...
        # Determine target clusters from arguments (before loading test data)
        ...
        # Run Pipeline with memory (no refined characteristics - always use seed)
        ...
        logging.info("Final predictions generation with seed characteristics (no refined) completed successfully!")
