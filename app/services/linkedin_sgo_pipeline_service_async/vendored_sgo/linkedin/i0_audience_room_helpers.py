"""
Audience-room LinkedIn **i0** helpers (prompts, topic scoring, deltas).

Used by ``run_linkedin_i0_only.py`` and ``tier1_delta_method_predictions.py`` — **not** dependent on
running the full tier1 delta script as an entrypoint.
"""
...
_PROMPTS_DIR = SCRIPTS_SGO / "prompts"
...
def measure_deltas(
    ...
    for retry in range(3):
        try:
            with emb_lock:
                pred_emb = emb_utils.get_or_compute_embedding(
                    pred_text,
                    review_key,
                    f"pred_{iter_suffix}",
                    sync_client,
                ...
                act_emb = emb_utils.get_or_compute_embedding(
                    act_text,
                    review_key,
                    "actual",
                    sync_client,
    ...
    theme_delta = float(get_theme_delta(pred_topics, gt_topic_probs, category_themes_list))
...
def build_prediction_snapshot_for_memory(
    user_pred_map: Dict[str, Dict[int, Dict[str, Any]]],
) -> Dict[Tuple[str, int], Dict[str, Any]]:
    """
    Snapshot ``best_prediction_data`` (else ``prediction``) per ``(profile_id, review_idx)``
    at outer-iteration boundaries. Part B / batch memory should treat this as the model output
    **before** the current iteration's sweep updates it (i.e. end of iteration ``i-1``).
    """
    out: Dict[Tuple[str, int], Dict[str, Any]] = {}
    for pid, rows in (user_pred_map or {}).items():
        for ridx, ent in (rows or {}).items():
            if not isinstance(ent, dict):
                continue
            best = ent.get("best_prediction_data") or ent.get("prediction") or {}
            if not isinstance(best, dict):
                best = {}
            themes = best.get("predicted_themes")
            if not isinstance(themes, dict):
                themes = {}
            out[(str(pid), int(ridx))] = {
                "review_text": str(best.get("review_text") or ""),
                "predicted_themes": copy.deepcopy(themes),
            }
    return out


def resolve_failed_prediction_for_memory_batch(
    *,
    profile_id: str,
    review_idx: int,
    entry: Dict[str, Any],
    snapshot_before_current_iteration: Dict[Tuple[str, int], Dict[str, Any]],
) -> Dict[str, Any]:
    """
    Prefer the iteration-start snapshot (previous iteration's final prediction); otherwise
    ``initial_prediction_data`` (cold start / first time).
    """
    key = (str(profile_id), int(review_idx))
    snap = snapshot_before_current_iteration.get(key)
    if isinstance(snap, dict):
        themes = snap.get("predicted_themes")
        rt = str(snap.get("review_text") or "")
        if rt or (isinstance(themes, dict) and themes):
            return {
                "review_text": rt,
                "predicted_themes": copy.deepcopy(themes) if isinstance(themes, dict) else {},
            }
    base = entry.get("initial_prediction_data") or entry.get("prediction") or {}
    if not isinstance(base, dict):
        base = {}
    bt = base.get("predicted_themes")
    return {
        "review_text": str(base.get("review_text") or ""),
        "predicted_themes": copy.deepcopy(bt) if isinstance(bt, dict) else {},
    }
...
async def run_audience_room_i0_for_post(
    *,
    ...
    """
    Single-post **i0** only (synthetic post + topic logprobs + deltas). No ``tier1_delta_method_predictions`` import.
    """
    ...
    entry = await run_i0_initial_prediction(
        ...
        append_all_reviews_delta=append_all_reviews_delta,
        scoring_mode=str(getattr(settings, "SCORING_MODE", "confidence") or "confidence"),
    )
