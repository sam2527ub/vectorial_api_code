"""Batched JSON calls for stimulus + category (tier 1) and category-only (tier 2)."""

from __future__ import annotations

import asyncio
import random
from typing import Any, Dict, Optional, Union

from openai import APIConnectionError, APIError, APITimeoutError, RateLimitError

from app.config import logger
from app.services.ai_gateway_service import ai_gateway
from app.services.linkedin_theme_discovery_service.tier_production_config import LlmRetryConfig
from .stimulus_extraction_config import get_stimulus_extraction_config
from .stimulus_constants import (
    _T1D,
    _T1F,
    _T2D,
    _T2F,
)


def _is_retryable(exc: BaseException) -> bool:
    if isinstance(exc, (RateLimitError, APIConnectionError, APITimeoutError, TimeoutError)):
        return True
    if isinstance(exc, APIError):
        code: Optional[int] = getattr(exc, "status_code", None)
        if code is not None and code in (429, 500, 502, 503, 504):
            return True
    msg = str(exc).lower()
    if "429" in msg or "rate limit" in msg or "timeout" in msg or "unavailable" in msg:
        return True
    return False


def _retry_after_from_exc(exc: BaseException) -> Optional[float]:
    if not isinstance(exc, APIError):
        return None
    resp: Any = getattr(exc, "response", None)
    if resp is not None and hasattr(resp, "headers") and resp.headers is not None:
        h: Any = resp.headers.get("retry-after")
        if h:
            try:
                return float(h)
            except (TypeError, ValueError):
                return None
    return None


async def call_stimulus_json_via_gateway(
    context_id: str,
    system: str,
    user: str,
    max_tokens: int,
    is_tier2: bool,
    model: str,
    llm_retry: LlmRetryConfig,
    temperature: Optional[float] = None,
    response_format: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    cfg = get_stimulus_extraction_config()
    temp = cfg.llm_temperature if temperature is None else temperature
    rfmt: Dict[str, Any] = (
        dict(cfg.response_format) if response_format is None else response_format
    )
    d_attr, f_attr = (_T2D, _T2F) if is_tier2 else (_T1D, _T1F)
    messages: list[dict[str, str]] = [
        {"role": "system", "content": system},
        {"role": "user", "content": user},
    ]
    last: Optional[BaseException] = None
    for attempt in range(1, llm_retry.max_attempts + 1):
        try:
            out: Union[dict[str, Any], str, Any] = await ai_gateway.call_via_gateway(
                context_id=f"{context_id}__a{attempt}",
                messages=messages,
                max_tokens=max_tokens,
                model=model,
                default_model=None,
                fallback_models=None,
                config_default_attr=d_attr,
                config_fallbacks_attr=f_attr,
                hardcoded_default="openai/gpt-4o-mini",
                validate_summary=False,
                return_text=False,
                direct_api_fallback_model=model,
                response_format=rfmt,
                temperature=temp,
            )
            if not isinstance(out, dict):
                raise TypeError(f"stimulus LLM: expected dict, got {type(out)}")
            return out
        except Exception as e:
            last = e
            if not _is_retryable(e) or attempt >= llm_retry.max_attempts:
                raise
            ra = _retry_after_from_exc(e)
            if ra is not None and ra > 0:
                delay = min(
                    llm_retry.max_delay_sec, ra + random.uniform(0, llm_retry.jitter_sec)
                )
            else:
                delay = min(
                    llm_retry.max_delay_sec,
                    llm_retry.base_delay_sec * (2 ** (attempt - 1))
                    + random.uniform(0, llm_retry.jitter_sec),
                )
            logger.warning(
                "stimulus extraction LLM attempt %s/%s failed: %s — retry in %.1fs",
                attempt,
                llm_retry.max_attempts,
                e,
                delay,
            )
            await asyncio.sleep(delay)
    if last is not None:
        raise last
    raise RuntimeError("stimulus LLM: exhausted retries")
