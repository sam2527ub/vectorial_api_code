...
class ApifySearchRequest(BaseModel):
    """Request for Apify LinkedIn Company Employees Scraper. camelCase to match Apify actor input."""
    ...
    maxItems: int = Field(
        100,
        ge=0,
        description="Max profiles to scrape (default 100); 0 = all (up to 2500 per query)",
    )
    companyBatchMode: Optional[str] = Field(
        None,
        description="'one_by_one' (up to 1000 companies) or 'all_at_once' (max 10)",
    )
    jobTitles: Optional[List[str]] = Field(None, description="List of job titles to filter (strict search)")
    locations: Optional[List[str]] = Field(None, description="List of locations (e.g. New York, London)")
    startPage: Optional[int] = Field(None, ge=0, description="Start from this search results page (0 = first page)")
    generalSearchQuery: Optional[str] = Field(None, description="Fuzzy search query")
    industryIds: Optional[List[int]] = Field(None, description="LinkedIn industry IDs")
    yearsAtCompany: Optional[List[int]] = Field(None, description="Years at company filter")
