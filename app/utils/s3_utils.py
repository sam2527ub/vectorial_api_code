...
def get_s3_key_for_audience(room_id: str, path: str, enterprise_name: Optional[str] = None, source: Optional[str] = None) -> str:
    ...
    # Get source-based audience path
    audience_path = get_source_audience_path(source)
    
    # Build the S3 key: {enterpriseName}/{source}-audience/{room_id}/{path}
    return f"{normalized_enterprise}/{audience_path}/{room_id}/{path}"
